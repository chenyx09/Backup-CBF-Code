function u = backup_CBF_con(x,u0,bdry)
global Ts vm rm am xx0 QQ0 tt0
...
for k=1:N_robot
%     [tt{k},xx{k},QQ{k}] = generate_backup_traj(x(:,k),con1,stop_crit);
[tt{k},xx{k},QQ{k}] = generate_backup_traj(x(:,k),con1,stop_crit);
% [f,g] = dubin(x(:,k));
% 
% x_dot_backup(:,k)  = f+g*con1(x(:,k));
%     [tt{k},xx{k},QQ{k}] = symmetry_backup(x(:,k),tt0,xx0,QQ0);
%     if x(3,k)>0.1
%         [tt1,xx1,QQ1] = generate_backup_traj(x(:,k),con1,stop_crit);
%     end
end
        ...
        for n = 1:N_robot
            if n~=k
                if j<=length(tt{n})
                    ...
                    [b,db] = two_robot_XY(xx{k}(j,:)',xx{n}(j,:)',r);
                    if b<-0.02
                        disp('')
                    end
                    if b<0.2
                        ...
                        bineq = [bineq;alpha*(b-0.01)+db'*QQ{k}(4*(j-1)+1:4*j,:)*f];
                    ...
                    [b,db] = two_robot_XY(xx{k}(j,:)',xx{n}(end,:)',r);
                    if b<-0.02
                        disp('')
                    end
                    if b<0.2
...
%         [res,~,exitflag] =qpOASES( diag([1 1 0]),[-u0(:,k);1e3],Aineq,[-am;-rm;0],[am;rm;inf],[],bineq,options);
        u(:,k)=res(1:2);
