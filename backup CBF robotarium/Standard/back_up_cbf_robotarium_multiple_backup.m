...
p_circ = 0.8*[xybound(2)*cos(p_theta) xybound(2)*cos(p_theta+pi); xybound(4)*sin(p_theta)  xybound(4)*sin(p_theta+pi)];
...
%                 end
    ...
    [u,backup_state] = backup_CBF_con_v1(t,x_e,u0,xybound,backup_state);
    u_rec = [u_rec;u];
    backup_state_rec = [backup_state_rec;backup_state'];
    dx = [v+u(1,:)*ts;u(2,:)];
    v = dx(1,:);
    for i=1:N
...
save('result_log','x_rec','u0_rec','u_rec','backup_state_rec');
